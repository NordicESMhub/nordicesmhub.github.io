---
layout: default
title: Programming
image: /images/tutorials/markus-spiske-221494-unsplash.jpg
---

# Beginner

- [Version control with Git](http://swcarpentry.github.io/git-novice/)
- [Programming with Python](http://swcarpentry.github.io/python-novice-inflammation/)
- [Geospatial Data Carpentry workshop](https://datacarpentry.org/geospatial-workshop/)
- [Introduction to NCL](https://annefou.github.io/ncl-novice/)
- [Introduction to Big-data using PySpark](https://annefou.github.io/pyspark/)
- [Introduction to the Internet of Things (IoT)](https://annefou.github.io/IoT_introduction/)

# Intermediate

- [A comprehensive introduction to version control with Git](https://coderefinery.github.io/git-intro/)
- [Lesson on archaeology with Git](https://coderefinery.github.io/git-archaeology/)
- [Collaborative distributed version control with Git](https://coderefinery.github.io/git-collaborative/)
- [Git branch design](https://coderefinery.github.io/git-branch-design/)
- [Programming with Fortran](http://annefou.github.io/Fortran/)
- [Working with Spatio-temporal Data in Python](https://annefou.github.io/metos_python/)
- [Reproducible Research with Interactive Jupyter Dashboards](https://annefou.github.io/jupyter_dashboards/)
- [Publication ready scientific reports and presentations with Jupyter notebooks](https://annefou.github.io/jupyter_publish/)
- [Learn Deep learning with Python](https://uiohive.github.io/deep-learning_intro/)

# Advanced

